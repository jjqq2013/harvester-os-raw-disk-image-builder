...
function kill_tmp_http_server() { ! HTTP_SERVER_PID=$(ss -ltnp 'sport = :10080' | grep -P -o '(?<=,pid=)\d+(?=,)' | head -n 1) || kill -9 $HTTP_SERVER_PID; }
...
rsync -av "$PROG_DIR"/$IMAGE_NAME/. "$RES_IMAGE".tmp/
